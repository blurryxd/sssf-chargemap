'use strict';
const connectionsSchema = require('../schemas/connectionsSchema');

const connections_list_get = async (req, res) => {
    try {
        const connections = await connectionsSchema.find().populate();
...
const connections_get = async (req, res) => {
    try {
        const connections = await connectionsSchema.findById(req.params.id);
        res.json(connections);
    } catch (e) {
